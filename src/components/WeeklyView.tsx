...
import { format, startOfWeek, endOfWeek, eachDayOfInterval, isSameDay, isToday, addWeeks, subWeeks, isSameWeek } from 'date-fns';
...
const WeeklyView: React.FC = () => {
    ...
    const [currentWeekStart, setCurrentWeekStart] = useState(() => startOfWeek(selectedDate, { weekStartsOn: 1 })); // 月曜日開始
    ...
    const weeklyStats = useMemo(() => {
        const allWeekRecords = weekDays.flatMap(day => getRecordsForDate(day));

        return {
            achievement: allWeekRecords.filter(r => r.category === 'achievement').length,
            happy: allWeekRecords.filter(r => r.category === 'happy').length,
            failure: allWeekRecords.filter(r => r.category === 'failure').length,
            trouble: allWeekRecords.filter(r => r.category === 'trouble').length
        };
    }, [weekDays, recordEvents, activeChildId]);

    // フィルタリングされた記録を取得
    const filteredRecords = useMemo(() => {
        const allWeekRecords = weekDays.flatMap(day => {
        ...
        if (selectedCategory) {
            return allWeekRecords.filter(record => record.category === selectedCategory);
        }
        return [];
    }, [weekDays, recordEvents, activeChildId, selectedCategory]);
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-base font-medium text-gray-800">週間サマリー</h3>
                    </div>
                    <button
                        onClick={handleThisWeek}
                        className="text-xs bg-orange-100 text-orange-600 px-2 py-1 rounded-md hover:bg-orange-200 transition-colors"
                    >
                        今週
                    </button>
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-sm font-medium text-gray-700">
                        {format(currentWeekStart, 'yyyy年MM月dd日', { locale: ja })} - {format(weekDays[6], 'MM月dd日', { locale: ja })}
                    </span>
                    <button
                ...
                <div className="grid grid-cols-4 gap-2 mb-4">
                    ...
                    >
                        ...
                        <div className="text-lg font-bold text-emerald-600">{weeklyStats.achievement}</div>
                    </button>
                    <button
                    ...
                    >
                        ...
                        <div className="text-lg font-bold text-sky-600">{weeklyStats.happy}</div>
                    </button>
                    <button
                    ...
                    >
                        ...
                        <div className="text-lg font-bold text-amber-600">{weeklyStats.failure}</div>
                    </button>
                    <button
                    ...
                    >
                        ...
                        <div className="text-lg font-bold text-rose-600">{weeklyStats.trouble}</div>
                    </button>
                </div>
            </div>
