...
const App: React.FC = () => {
  ...
  const [showMonthlyRecordsModal, setShowMonthlyRecordsModal] = useState(false);
  const [showMonthlyAttendanceModal, setShowMonthlyAttendanceModal] = useState(false);
      ...
      case 'attendance':
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
              <div className="p-6 border-b border-gray-100">
                ...
                <div className="flex justify-end">
                  <button
                    onClick={() => setShowMonthlyAttendanceModal(true)}
                    className="flex items-center gap-2 px-4 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-colors shadow-md"
                  >
                    <Printer size={18} />
                    <span className="text-sm font-medium">個人別月次出席記録</span>
                  </button>
                </div>
              </div>
            ...
            {/* 個人別月次出席記録モーダル */}
            {showMonthlyAttendanceModal && user && (
              <MonthlyAttendanceByChild
                facilityId={user.id}
                onClose={() => setShowMonthlyAttendanceModal(false)}
              />
            )}
          </div>
        );
        ...
        return (
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-3">
                <button
                  onClick={() => setShowMonthlyRecordsModal(true)}
                  className="flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition-colors shadow-md"
                >
                  <Printer size={18} />
                  <span className="text-sm font-medium">個人別月次記録</span>
                </button>
            ...
            {/* 個人別月次記録モーダル */}
            {showMonthlyRecordsModal && user && (
              <MonthlyRecordsByChild
                facilityId={user.id}
                onClose={() => setShowMonthlyRecordsModal(false)}
              />
            )}
          </div>
        );
